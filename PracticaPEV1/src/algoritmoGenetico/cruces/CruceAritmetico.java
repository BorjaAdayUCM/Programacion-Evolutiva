...
public class CruceAritmetico extends Cruce {
	...
	public CruceAritmetico(Individuo[] poblacion, int tamPoblacion, Random rand, double probCruce, int numProblema) {
		...
		for(int i = 0; i < this.tamPoblacion - 1; i+=2) {
			if(this.rand.nextDouble() < this.probCruce) {
				...
				for(int j = 0; j < tamCromosoma; j++) {
					nuevaPoblacion[i].getCromosoma()[j] = ((Double) this.poblacion[i].getCromosoma()[j] + (Double) this.poblacion[i + 1].getCromosoma()[j]) / 2;
					double p = this.rand.nextDouble();
					nuevaPoblacion[i + 1].getCromosoma()[j] = (((Double) this.poblacion[i].getCromosoma()[j]) * p) + (((Double) this.poblacion[i + 1].getCromosoma()[j]) * (1 - p));
				}
			}
		}
